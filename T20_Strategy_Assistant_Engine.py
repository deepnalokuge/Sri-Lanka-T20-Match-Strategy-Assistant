...
if st.button("🚀 Generate Match Strategy"):
    ...
    if toss_result == "Won":
        ...
        st.markdown(" Toss Strategy: <span style='color:red'>Decision by Opponent (Lost)</span>", unsafe_allow_html=True)

    st.subheader("📊 Phase-wise Game Plan")


    # ================================
    # Helper for phase strategy
    # ================================
    ...
    # ================================
    # ADVANCED CHASING LOGIC (Corrected)
    # ================================
    if opponent_score > 0:
        ...
        # FIXED: distribute runs based on required RR
    ...
    if total_predicted == 0:
        ...
        normalized_roles = {role: round((count / total_predicted) * 11)
                            for role, count in role_preds.items()}
    ...
    players_df = players_df[players_df['role'].isin([
        'batter', 'fast bowler', 'spinner', 'allrounder'
    ])]

    prob = LpProblem("Best_XI_Selection", LpMaximize)

    player_vars = {
        row['player']: LpVariable(row['player'], cat='Binary')
        for _, row in players_df.iterrows()
    }

    prob += lpSum(
        player_vars[player] *
        players_df.loc[players_df['player'] == player, 'score'].values[0]
        for player in player_vars
    )

    prob += lpSum(player_vars.values()) == 11

    wicketkeepers = players_df[(players_df['is_wk'] == 1) &
                               (players_df['role'] == 'batter')]['player'].tolist()
    ...
    for role_name, count in final_roles.items():
        players_in_role = [
            p for p in player_vars
            if players_df.loc[players_df['player'] == p, 'role'].values[0]
            == role_map[role_name]
        ]
        if players_in_role:
            prob += lpSum([player_vars[p] for p in players_in_role]) == count

    prob.solve()

    selected_players = [
        p for p in player_vars if player_vars[p].varValue == 1.0
    ]
    ...
    for phase in ["powerplay", "middle", "death"]:
        strategy, field_positions = generate_field_setting(input_data, phase)
