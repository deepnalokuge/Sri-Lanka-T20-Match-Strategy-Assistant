...
if st.button("🚀 Generate Match Strategy"):
    ...
    # ================================
    # Phase-wise Bowling Strategy (Improved)
    # ================================
    if toss_result == "Lost" or (toss_result == "Won" and toss_pred == 0):

        st.subheader("🛡️ Phase-wise Bowling Strategy")

        # Helper function
        def show_bowling_phase(name, conc_model, wkts_model, strat_model):
            try:
                conc = int(conc_model.predict(base_df)[0])
            except:
                conc = 0

            try:
                wkts = int(wkts_model.predict(base_df)[0])
            except:
                wkts = 0

            try:
                strat = strat_model.predict(base_df)[0]
            except:
                strat = "Standard bowling plan"

            st.markdown(f"### {name}")
            st.markdown(f"- **Expected Runs Conceded**: `{conc} runs`")
            st.markdown(f"- **Expected Wickets Taken**: `{wkts} wickets`")
            st.markdown(f"- **Strategy**: <span style='color:green'>{strat}</span>", unsafe_allow_html=True)
            return conc

        # ---- Each Phase ----
        conc_pp = show_bowling_phase(
            "Powerplay (1–6 overs)",
            pp_conc_model,
            pp_wkts_model,
            powerplay_model
        ...
        # ---- Total Restriction Target ----
        total_restrict = conc_pp + conc_mo + conc_do

        st.markdown(f"### 🔢 **Expected Total to Restrict Opponent:** `{total_restrict} runs`")
