...
opponent_score = st.sidebar.number_input(
    "Opponent's 1st Innings Score \\n(leave 0 if not applicable)", min_value=0, max_value=300, step=1, value=0
)
...
if st.button("🚀 Generate Match Strategy"):
    ...
    # ==============================================
    # COMBINED BOWLING STRATEGY (with model labels)
    ...
    if (toss_result == "Won" and toss_decision_val == "bowl") or (toss_result == "Lost"):
        ...
        try:
            ...
            st.error(f"❌ Bowling prediction models failed: {e}")
    ...
    #st.markdown(f"### 🏏 Expected Total to Restrict Opponent: **{int(total_conceded_pred)} runs**")
        ...
        st.markdown(f"- **Strategy:** <span style='color:green; font-weight:bold;'>{pp_bowl_str}</span>", 
                    unsafe_allow_html=True)
        ...
        st.markdown(f"- **Strategy:** <span style='color:green; font-weight:bold;'>{mo_bowl_str}</span>", 
                    unsafe_allow_html=True)
        ...
        st.markdown(f"- **Strategy:** <span style='color:green; font-weight:bold;'>{do_bowl_str}</span>", 
                    unsafe_allow_html=True)
