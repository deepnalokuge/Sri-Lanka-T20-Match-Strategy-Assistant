...
if st.button("🚀 Generate Match Strategy"):
    ...
    # ================================
    #  NEW BOWLING STRATEGY SECTION  
    #  (FOR BOTH TOSS WIN & TOSS LOSS)
    # ================================

    # Load your NEW bowling models
    ...
    # CONDITIONS TO SHOW BOWLING STRATEGY:
    # 1. SL wins toss & chooses bowl
    # 2. SL loses toss (opponent decides SL to bowl)

    if (toss_result == "Won" and toss_decision_val == "bowl") or (toss_result == "Lost"):
        st.subheader("🛡️ Bowling Strategy (SL Bowling Innings)")

    # Phase-wise conceded predictions
    try:
        conc_pp = pp_conc_model.predict(base_df)[0]
        conc_mo = mo_conc_model.predict(base_df)[0]
        conc_do = do_conc_model.predict(base_df)[0]
    except:
        st.error("❌ Bowling conceded-run models failed. Check dataset columns.")
        conc_pp = conc_mo = conc_do = 0

    total_conceded_pred = conc_pp + conc_mo + conc_do

    # Display summary
    st.markdown(f"### 🔢 Expected Total Runs to Restrict: **{int(total_conceded_pred)}**")

    st.markdown("### 🎯 **Powerplay (1–6 overs)**")
    st.markdown(f"- Expected conceded runs: **{int(conc_pp)}**")
    st.markdown("- Bowlers: Swing bowlers, Test length, 1–2 slips.")
    st.markdown("- Target: Keep run rate under 7.0 RPO.")

    st.markdown("### 🎯 **Middle Overs (7–15 overs)**")
    st.markdown(f"- Expected conceded runs: **{int(conc_mo)}**")
    st.markdown("- Use spin choke: tight lines, defensive fields.")
    st.markdown("- Target: Build pressure with dot balls.")

    st.markdown("### 🎯 **Death Overs (16–20 overs)**")
    st.markdown(f"- Expected conceded runs: **{int(conc_do)}**")
    st.markdown("- Use death specialists only.")
    st.markdown("- Mix yorkers, slower balls, wide lines to big hitters.")
    st.markdown("- Target: Under 40 runs in last 5 overs.")
