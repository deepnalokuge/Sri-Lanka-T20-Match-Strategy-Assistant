...
if st.button("🚀 Generate Match Strategy"):
    ...
    # Decide when to show batting strategy
    show_batting = False

    if toss_result == "Won":
        if toss_pred == 1:  # Bat first
            show_batting = True
    else:
        # Toss Lost → SL still needs batting plan
        show_batting = True


    # --- Define function OUTSIDE conditions ---
    ...
    # --- Show batting strategy when needed ---
    if show_batting:
    ...
    # Chase difficulty classification
        if target_score < 140:
    ...
    # ==============================================
    # COMBINED BOWLING STRATEGY (Wickets + Runs)
    # For: Toss Won & Bowl  OR Toss Lost
    # ==============================================
    ...
    # ---- Predictions ----
        ...
        # Conceded runs
            ...
            st.error("❌ Bowling prediction models failed. Check training features.")
        ...
        # ---- Summary ----
        st.markdown(f"###  Expected Total to Restrict Opponent: **{int(total_conceded_pred)} runs**")

        # ---- POWERPLAY ----
        st.markdown("###  Powerplay (1–6 overs)")
        st.markdown(f"- **Expected Wickets:** {int(pp_wkts_b)}")
        st.markdown(f"- **Expected Conceded Runs:** {int(conc_pp)}")
        st.markdown("**Plan:**")
        st.markdown("- Use attacking field (1–2 slips)")
        st.markdown("- Swing bowlers, test length, hit the hard length")
        st.markdown("- Target: Keep RPO under 7.0")

        # ---- MIDDLE OVERS ----
        st.markdown("###  Middle Overs (7–15 overs)")
        st.markdown(f"- **Expected Wickets:** {int(mo_wkts_b)}")
        st.markdown(f"- **Expected Conceded Runs:** {int(conc_mo)}")
        st.markdown("**Plan:**")
        st.markdown("- Use spin choke (tight lines, defensive fields)")
        st.markdown("- Maintain pressure with dot balls")

        # ---- DEATH OVERS ----
        st.markdown("###  Death Overs (16–20 overs)")
        st.markdown(f"- **Expected Wickets:** {int(do_wkts_b)}")
        st.markdown(f"- **Expected Conceded Runs:** {int(conc_do)}")
        st.markdown("**Plan:**")
        st.markdown("- Use death specialists only")
        st.markdown("- Mix yorkers, slower balls, wide lines")
