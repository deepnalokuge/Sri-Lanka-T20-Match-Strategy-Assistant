...
if st.button("🚀 Generate Match Strategy"):
    ...
    # NEW: BOWLING-FIRST STRATEGY (USING TRAINED WKTS MODELS)
    # ========================================

    if toss_result == "Won" and toss_decision_val == "bowl":
        st.subheader("🎯 Bowling First Strategy (SL Bowling Innings 1)")

        # Predict wickets SL can take phase-wise
        try:
            pp_wkts_bowl = pp_wkts_model.predict(base_df)[0]
            mo_wkts_bowl = mo_wkts_model.predict(base_df)[0]
            do_wkts_bowl = do_wkts_model.predict(base_df)[0]
        except:
            st.error("Wicket prediction models failed. Check training features.")
            pp_wkts_bowl = mo_wkts_bowl = do_wkts_bowl = 0

        st.markdown("###  **Powerplay (1–6 overs)**")
        st.markdown(f"- Expected wickets: **{int(pp_wkts_bowl)}**")
        st.markdown("- Use attacking field (1–2 slips).")
        st.markdown("- Bowlers: New-ball swing bowlers, hit hard length.")

        st.markdown("###  **Middle Overs (7–15 overs)**")
        st.markdown(f"- Expected wickets: **{int(mo_wkts_bowl)}**")
        st.markdown("- Use spinners in tandem.")
        st.markdown("- Keep tight fields, force risky shots.")

        st.markdown("###  **Death Overs (16–20 overs)**")
        st.markdown(f"- Expected wickets: **{int(do_wkts_bowl)}**")
        st.markdown("- Use best death specialists.")
        st.markdown("- Mix yorkers, slower balls, wide lines to big hitters.")

        # ================================
# BOWLING STRATEGY (NEW FEATURE)
# ================================
try:
    bowl_model = joblib.load("bowling_strategy.pkl")

    # Predict allowed score if SL bowls first
    predicted_conc_runs = bowl_model.predict(base_df)[0]
    predicted_conc_runs = int(predicted_conc_runs)

    st.subheader("🎯 Bowling Strategy Recommendation")

    if toss_result == "Won":
        if toss_decision_val == "bowl":
            st.markdown(f"**Sri Lanka Bowling Objective:** Restrict opponent to **{predicted_conc_runs} runs**")
        else:
            st.info("SL chose to BAT — displaying batting strategy.")
    else:
        # SL lost toss → Opponent chooses
        st.markdown("**Sri Lanka likely to BOWL first (opponent decision).**")
        st.markdown(f"➡️ **Bowling Objective:** Restrict opponent to **{predicted_conc_runs} runs**")

    # Optional breakup of bowling phases (if you want phase-wise)
    if 'pp_conc_model.pkl' in os.listdir():
        pp_conc = int(joblib.load("pp_conc_model.pkl").predict(base_df)[0])
        mo_conc = int(joblib.load("mo_conc_model.pkl").predict(base_df)[0])
        do_conc = int(joblib.load("do_conc_model.pkl").predict(base_df)[0])

        st.markdown("### 🔍 Phase-wise Bowling Target")
        st.markdown(f"- **Powerplay:** Restrict to **{pp_conc}** runs")
        st.markdown(f"- **Middle Overs:** Restrict to **{mo_conc}** runs")
        st.markdown(f"- **Death Overs:** Restrict to **{do_conc}** runs")

except:
    st.warning("⚠ Bowling strategy model not available. Please train bowling_strategy.pkl")
